...
1. Choose the target.
   ...
   b. Otherwise take the first entry in factory/queue.json whose status is "queued". If it needs a server or an external library, set its status to "blocked" with a one-line "why", and take the next one.
...
4. Log. Append events to journal/events.json with real UTC timestamps (types: request, build, test, release; release events carry "link": "apps/<slug>.html"). Set the queue item's status to "done" and add "shipped": "<YYYY-MM-DD>". Keep journal/events.json valid JSON.
