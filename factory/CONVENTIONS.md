...
- A new tool is one file: apps/<slug>.html, following apps/tasks.html exactly: NOL.topbar('<slug>'), .app-head with title and toolbar (search, Import CSV, Export CSV, primary + button), empty state via NOL.empty, <dialog> forms, NOL.store collections, CSV import through NOL.csvToObjects + NOL.mapHeaders with a synonym spec covering the headers of the tools it replaces, addEventListener("nol:change", () => render()) before the final render().
   ...
   - Add the products it replaces to data/saas.json: new "cat" equal to the app slug, approximate public list price in USD per user per month (or "flat": true per month), a "tier" note, aliases. Catalogue prices are approximate public list prices by design, not verified quotes: write the closest number you know, and put `"tier": "estimate"` when you are unsure which plan it is (many entries already do). A price is the one world fact you may decide yourself — never stop the task to ask the owner about it. Add the category to the APP map in scripts/build.mjs and to the APP object in unsubscribe.html.
