...
// Five agents plus testers write to the board at once, so GitHub rejects stale shas (409/422).
// fn must re-read the file (fresh sha), re-apply the change and write; on a sha conflict it is
// re-run up to 6 more times with growing backoff (300, 600, 1200ms…) before failing for real.
const rmw = async fn => { for (let i = 0; ; i++) { try { return await fn(); } catch (e) { if ((e.status !== 409 && e.status !== 422) || i >= 6) throw e; await new Promise(r => setTimeout(r, 300 * 2 ** i)); } } };
async function update(id, patch, msg) {
  return rmw(async () => {
    ...
    await write(tasks, sha, msg); return t;
  });
}
...
if (cmd === 'seed') {
  ...
  await rmw(async () => { const { tasks: cur, sha: curSha } = await read(); await write(cur.concat(shipped, queued), curSha, 'factory: seed backlog'); }); console.log('seeded', shipped.length, 'done +', queued.length, 'queued →', REPO);
} else if (cmd === 'list') {
...
else if (cmd === 'add') { const [title, spec] = rest.filter(r => r !== '--top'); await rmw(async () => { const { tasks, sha } = await read(); const t = { id: 'factory-' + id, created: now(), title, slug: id, improve: '', status: 'Queued', project: 'Factory', assignee: 'Factory agent', priority: rest.includes('--top') ? 'high' : '', due: '', description: spec || '' }; if (rest.includes('--top')) t.rank = minRank(tasks) - 1; tasks.push(t); await write(tasks, sha, `factory: add ${t.id}`); }); console.log('added', 'factory-' + id, rest.includes('--top') ? '(top of queue)' : ''); }
else if (cmd === 'top') { await rmw(async () => { const { tasks, sha } = await read(); const k = tasks.findIndex(x => x.id === id); if (k < 0) throw new Error('no task ' + id); tasks[k].rank = minRank(tasks) - 1; tasks[k].updated = now(); await write(tasks, sha, `factory: top ${id}`); }); console.log('top', id); }
else if (cmd === 'note') { const { tasks } = await read(); if (!tasks.find(x => x.id === id)) throw new Error('no task ' + id); await rmw(async () => { const f = await api('GET', `/repos/${REPO}/contents/notes.json?ref=main`); const notes = f ? JSON.parse(Buffer.from(f.content, 'base64').toString('utf8')) : []; notes.push({ id: crypto.randomUUID(), created: now(), coll: 'tasks', ref: id, text: note.slice(0, 4000), author: 'Тестировщик' }); const body = { message: `qa: note ${id}`, content: Buffer.from(JSON.stringify(notes)).toString('base64'), branch: 'main' }; if (f) body.sha = f.sha; await api('PUT', `/repos/${REPO}/contents/notes.json`, body); }); console.log('note added'); }
else if (cmd === 'upload') { const dir = id; const { readdirSync, readFileSync } = await import('node:fs'); const urls = []; for (const f of readdirSync(dir)) { if (!/\.(png|json|md|txt)$/.test(f)) continue; const pathq = `qa/${dir.split('/').pop()}/${f}`; await rmw(async () => { const cur = await api('GET', `/repos/${REPO}/contents/${pathq}?ref=main`); const body = { message: `qa: ${pathq}`, content: readFileSync(`${dir}/${f}`).toString('base64'), branch: 'main' }; if (cur) body.sha = cur.sha; await api('PUT', `/repos/${REPO}/contents/${pathq}`, body); }); urls.push(`https://github.com/${REPO}/blob/main/${pathq}`); } console.log(urls.join('\n')); }
