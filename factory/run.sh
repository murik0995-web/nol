...
if node --test >> "$LOG" 2>&1 && node scripts/build.mjs >> "$LOG" 2>&1 && python3 -c "import json;json.load(open('journal/events.json'));json.load(open('factory/queue.json'))" >> "$LOG" 2>&1; then
  git add -A
