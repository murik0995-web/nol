...
if [ -f .always ]; then
  ...
  case "$IN" in
    ...
    *ХОРОШО*)  # свой файл: не конфликтует с другими задачами; «Гость» — не ГОСТ, гейт фактов молчит
      printf '// Гость видит только CRM, Гостиница — просто слово\nexport const ok = 1\n' > ok.mjs ;;
...
try {
  ...
  assert.ok(/ссылка на норматив/.test(task('ВЫДУМКА').error || ''), 'ГОСТ Р 52242-2004 — ссылка на норматив, гейт обязан её назвать')
