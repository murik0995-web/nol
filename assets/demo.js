...
(function () {
  ...
  function load() {
    ...
    const pageRecs = pages.map(([title, folder, body]) => add('pages', { title, folder, body })); // Training builds its lessons out of these very pages
    ...
    const trCourses = (ru ? [
      ['Первая неделя в компании', 'Для всех', 70, [
        [0, 6, [['Где фиксируются все договорённости?', ['В переписке', 'В вики', 'Нигде, мы помним'], 1], ['Как приходят вопросы клиентов?', ['Через поддержку', 'На личную почту', 'Через руководителя'], 0]]],
        [3, 3, [['Кто может остановить релиз?', ['Только директор', 'Только релиз-менеджер', 'Любой в команде'], 2]]],
        [1, 4, [['За сколько подают заявку на отпуск?', ['Минимум за две недели', 'За день', 'За месяц'], 0], ['Сколько дней отпуска в год?', ['21', '28', '35'], 1]]],
      ]],
      ['Счета без ошибок', 'Финансы', 80, [
        [2, 5, [['Откуда берутся данные клиента?', ['Из CRM', 'Вводим руками каждый раз', 'Из выписки банка'], 0], ['Что нажать после оплаты?', ['Ничего', 'Отметить оплаченным', 'Удалить счёт'], 1]]],
        [4, 2, []],
      ]],
    ] : [
      ['First week at the company', 'Everyone', 70, [
        [0, 6, [['Where does every agreement go?', ['Into a chat message', 'Into the Wiki', 'Nowhere, we remember it'], 1], ['How do customer questions reach us?', ['Through Desk', 'By personal email', 'Through the manager'], 0]]],
        [3, 3, [['Who can stop a release?', ['Only the CEO', 'Only the release manager', 'Anyone on the team'], 2]]],
        [1, 4, [['How far ahead is a time-off request made?', ['At least two weeks', 'The day before', 'A month'], 0], ['How many days off a year?', ['21', '28', '35'], 1]]],
      ]],
      ['Invoicing without mistakes', 'Finance', 80, [
        [2, 5, [['Where do the client details come from?', ['From CRM', 'Typed by hand every time', 'From the bank statement'], 0], ['What do you click once it is paid?', ['Nothing', 'Mark paid', 'Delete the invoice'], 1]]],
        [4, 2, []],
      ]],
    ]).map(([title, audience, pass, ls]) => add('courses', { title, audience, pass, lessons: ls.map(([pi, minutes, qs]) => ({ id: NOL.id(), title: pageRecs[pi].title, pageId: pageRecs[pi].id, minutes, quiz: qs.map(([q, opts, a]) => ({ q, opts, a })) })) }));
    // кто на каком курсе: один прошёл всё, двое новичков в процессе, у бухгалтера срок уже вышел
    (ru ? [['Наталья Егорова', 0, 14, 2], ['Павел Орлов', 0, 21, 0], [people[3].name, 1, -3, 1], [people[1].name, 0, -10, 3]]
      : [['Natalia Egorova', 0, 14, 2], ['Pavel Orlov', 0, 21, 0], [people[3].name, 1, -3, 1], [people[1].name, 0, -10, 3]])
      .forEach(([person, ci, dd, n]) => {
        const c = trCourses[ci], done = {};
        c.lessons.slice(0, n).forEach((l, k) => { done[l.id] = { at: D(-7 + k * 2), right: l.quiz.length, of: l.quiz.length } });
        add('enrollments', { person, courseId: c.id, due: D(dd), started: D(-9), done });
      });
