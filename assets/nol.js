...
(function (root) {
  ...
  /* ---------- recurring tasks: a task marked repeating spawns the next occurrence when it is ticked off ---------- */
  const REPEATS = ['daily', 'weekly', 'monthly'];
  const stepDate = (iso, rule, anchor) => rule === 'monthly' ? addMonths(iso, 1, anchor) : day(Date.parse(iso) + (rule === 'weekly' ? 7 : 1) * 864e5);
  function nextTask(t, t0) {                                                      // the fields of the next occurrence; the caller decides which column it lands in
    if (!t || !REPEATS.includes(t.repeat)) return null;
    t0 = t0 || day();
    const base = t.due || t.start || t0, anchor = +String(base).slice(8, 10) || 0; // the 31st keeps landing on the 31st instead of sliding to the 28th after one short month
    let next = stepDate(base, t.repeat, anchor);
    for (let g = 0; g < 400 && next && next <= t0; g++) next = stepDate(next, t.repeat, anchor); // ticked off weeks late: the next one lands ahead, not in the past
    if (!next) return null;
    const off = Math.round((Date.parse(next) - Date.parse(base)) / 864e5);
    return {
      title: t.title, assignee: t.assignee || '', priority: t.priority || '', project: t.project || '', description: t.description || '',
      start: t.start ? day(Date.parse(t.start) + off * 864e5) : '',                // the gap between start and due travels with the task
      due: t.start && !t.due ? '' : next,
      subs: (Array.isArray(t.subs) ? t.subs : []).map(s => ({ text: s.text, done: false })), // a fresh occurrence starts with every step unticked
      repeat: t.repeat, repeatOf: t.id, order: t.order,
    };
  }
  ...
  const norm = s => String(s).toLowerCase().replace(/[^\p{L}\p{N}]+/gu, ''); // Unicode letters/digits: an ASCII-only strip turns any Cyrillic header (Bitrix24, amoCRM) into '' and every field would "match" it
  ...
  /* ---------- whiteboard geometry: text broken into lines an SVG can draw, and the point where an arrow meets the box it points at ---------- */
  function wrapText(text, max) {
    ...
    for (const para of String(text ?? '').split('\n')) {
      ...
      for (let word of para.split(/\s+/)) {
        if (!word) continue;
        while (word.length > max) { if (line) { out.push(line); line = ''; } out.push(word.slice(0, max)); word = word.slice(max); } // a pasted URL nobody can break stays inside the note instead of running off the board
        if (!line) line = word;
  ...
  function boxEdge(box, tx, ty) {                                                 // an arrow stops on the border of the note it points at, not in the middle of the text
    const cx = (+box.x || 0) + (+box.w || 0) / 2, cy = (+box.y || 0) + (+box.h || 0) / 2, dx = tx - cx, dy = ty - cy;
    if (!dx && !dy) return { x: cx, y: cy };
    const sx = Math.abs(dx) > 1e-9 ? Math.abs((+box.w || 0) / 2 / dx) : Infinity;
    const sy = Math.abs(dy) > 1e-9 ? Math.abs((+box.h || 0) / 2 / dy) : Infinity;
    const s = Math.min(sx, sy, 1);                                                // a target inside the box is the answer itself: no overshooting arrow head
...
@media print{body{background:#fff;color:#111}.ver{background:#111;color:#fff}.body a{color:#111}article,header{border-color:#ddd}.body code,.body pre{background:#f4f4f5}}`;
  ...
  /* ---------- workspace currency: one record in the synced 'settings' collection; display only, amounts are never converted. Never derived from the interface language: switching to Russian must not relabel dollars as roubles ---------- */
  const CURRENCIES = ['USD', 'EUR', 'GBP', 'RUB', 'UAH', 'KZT', 'BYN', 'AMD', 'GEL', 'TRY', 'AED', 'CNY', 'JPY', 'INR', 'PLN', 'CHF', 'CAD', 'AUD', 'BRL'];
  const currency = () => { const s = store.get('settings', 'workspace'); return (s && s.currency) || 'USD'; };
  ...
  const SECTIONS = [
    ...
    ['Work', [['tasks', 'Tasks'], ['goals', 'Goals'], ['wiki', 'Wiki'], ['mindmaps', 'Mind maps'], ['helpcenter', 'Help center'], ['meetings', 'Meetings'], ['standups', 'Standups'], ['retros', 'Retros'], ['whiteboard', 'Whiteboard'], ['feedback', 'Feedback'], ['roadmap', 'Roadmap'], ['changelog', 'Changelog']]],
...
footer a{color:var(--acid)}`;
  ...
  const SEARCH = {
    ...
    boards: { label: 'Board', title: r => r.name, sub: () => '', extra: () => [], url: r => 'whiteboard.html#open=' + r.id },
    shapes: { label: 'Sticky note', title: r => r.text || '', sub: r => (store.get('boards', r.boardId) || {}).name || '', extra: r => r.text ? [r.author, (store.get('boards', r.boardId) || {}).name] : [], url: r => 'whiteboard.html#open=' + r.id },
