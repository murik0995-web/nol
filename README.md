...
| `apps/whiteboard.html` | Miro, Mural, FigJam, Stormboard… | sticky note / board CSV |
...
- **Whiteboard** — an infinite canvas for the part of the work that happens before a task exists: sticky notes, plain text headings and arrows between them, dragged around with the mouse, panned by dragging the canvas and zoomed with the wheel. A note is as tall as its own text, so nothing ever spills over an edge; six colours, an author from People and a vote count sit on the note itself. Dragging the dot on a note's edge onto another note draws an arrow between them, and the arrow follows both notes wherever they go. **Export SVG** writes the whole board as one plain SVG file — the notes, the text and the arrows as real vector shapes, no scripts, no fonts to install, opens in any browser and prints at any size. A note that turns into work becomes a real task in Tasks, with its author as the assignee and the board's name as the project. A CSV from Miro, Mural, FigJam or Stormboard imports in one click: a frame or section column becomes a board of its own, sticky colours are matched by name, votes come across, and notes without coordinates are laid out on a grid.
