...
- **Mind maps** — a map that lays itself out. Nothing stores a coordinate: every node knows its parent, and the picture is computed from the tree, so the branches move over to make room by themselves. It is built from the keyboard the way thinking goes — **Enter** for the next thought, **Tab** for one level in, **Shift+Tab** for one level out, **F2** to rename, **Alt+↑↓** to reorder, the arrows to walk the map — and a branch dragged onto another node takes its whole subtree with it. Branches alternate left and right of the centre, each in its own colour, and any branch collapses to show the shape instead of the detail. A node carries a note for what does not fit on a line, and becomes a real task in Tasks in one click. **Export SVG** writes the map as one standalone file: no scripts, no fonts from the network, sharp at any size. An outline CSV from XMind, MindMeister, Coggle, MindManager or Mindomo imports in one click, in every shape those tools write — a column per level, a numeric level column, or a parent column naming its parent — and what NOL exports comes back as the same map.
