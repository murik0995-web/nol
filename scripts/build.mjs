...
const APP = { crm: ['CRM', 'contacts, companies and a deal pipeline', 'Export your contacts and deals as CSV'], desk: ['Desk', 'tickets, replies, priorities and statuses', 'Export your tickets as CSV'], people: ['People', 'an employee directory, teams and time off', 'Export your employee directory as CSV'], orgchart: ['Org chart', 'an org chart drawn from the manager field in People, with collapse, print and a missing-manager report', 'Export your directory with its manager column as CSV'], leave: ['Leave', 'a month calendar of time off, public holidays and who is out today', 'Export your leave or absence report as CSV'], wiki: ['Wiki', 'Markdown pages with folders and search', 'Export your pages as Markdown'], helpcenter: ['Help center', 'a public help center generated from your Wiki folder: static HTML with search, downloaded as files or as one page', 'Export your articles as CSV'], meetings: ['Meetings', 'an agenda, Markdown notes, a decisions log and action items that become tasks', 'Export your meetings as CSV'], tasks: ['Tasks', 'boards, lists, assignees and due dates', 'Export your board or project as CSV'], goals: ['Goals', 'objectives and key results by quarter, with a weighted rollup and check-ins', 'Export your objectives and key results as CSV'], quotes: ['Quotes', 'quotes and proposals from your own price list, with discounts, statuses and print-to-PDF', 'Export your quotes as CSV'], invoices: ['Invoices', 'invoices with payments, balance due, recurring drafts and print-to-PDF', 'Export your invoices as CSV'], contracts: ['Contracts', 'contract templates with placeholders, renewal and notice dates, statuses and print-to-PDF', 'Export your contracts as CSV'], expenses: ['Expenses', 'an expense log with categories, merchants, monthly totals and bank imports', 'Export your expenses as CSV'], inventory: ['Inventory', 'items with SKUs, quantities, locations, reorder levels and a stock movements log', 'Export your items as CSV'], assets: ['Assets', 'an IT and equipment register with serial numbers, holders, purchase dates and warranty ends', 'Export your asset register as CSV'], reviews: ['Reviews', 'performance review cycles with a self review and a manager review on one form, ratings, 1:1 notes and the history per person', 'Export your review cycle as CSV'], hiring: ['Hiring', 'jobs, candidates and a stage board with drag and drop', 'Export your candidates as CSV'], standups: ['Standups', 'async daily check-ins with your own questions, answers per person and date, history and a blockers filter', 'Export your standup history as CSV'], retros: ['Retros', 'retrospective boards with votes on every card, action items that become real tasks and an archive of every retro you ran', 'Export your retro board as CSV'], roadmap: ['Roadmap', 'a Now / Next / Later roadmap board with items linked to real tasks and a public roadmap published as one static HTML file', 'Export your roadmap as CSV'], changelog: ['Changelog', 'product updates in Markdown with tags and a standalone HTML page you can publish anywhere', 'Export your updates or release notes as CSV'], subscriptions: ['Subscriptions', 'a vendor and subscription tracker with renewal alerts', 'Export your subscriptions as CSV'], timesheets: ['Time', 'timers, a weekly timesheet grid and per-person totals', 'Export your detailed time report as CSV'] };
...
// price == null: this catalogue has no list price for that product yet. Never guess one — say so, and let the page ask for a correction.
const price = p => p.price == null ? 'price on request' : p.price === 0 ? 'free tier' : p.flat ? `$${p.price}/mo flat` : `$${p.price}/user/mo`;
const yearly = (p, n) => p.price == null ? null : p.price === 0 ? 0 : p.flat ? p.price * 12 : p.price * n * 12;
const yTile = v => v == null ? '—' : '$' + v.toLocaleString('en-US');
...
<p class="lead" style="margin-top:18px">${p.price == null ? `The list price of ${esc(p.name)} is <b>not in this catalogue yet</b>.` : `${esc(p.name)} lists at <b>${price(p)}</b>.`} NOL ${app} gives you ${what} for <b class="acid">$0</b>, open source, running in your browser today. Your data stays yours.</p>
...
<div class="card"><div class="kpi">${yTile(y25)}<small>${esc(p.name)}, 25 people, per year</small></div></div>
<div class="card"><div class="kpi">${yTile(y100)}<small>${esc(p.name)}, 100 people, per year</small></div></div>
...
<tr><td>Per-seat tax</td><td>${p.price == null ? '—' : p.flat ? 'tiered' : 'yes'}</td><td>none</td></tr>
