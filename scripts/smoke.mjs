...
await new Promise(r => srv.listen(0, '127.0.0.1', r)); const base = `http://127.0.0.1:${srv.address().port}/`; // 127.0.0.1, not localhost: the conveyor sandbox resolves localhost to ::1 and requests hang
...
for (const lang of ['en', 'ru']) {
  ...
  const chrome = spawn(CHROME, ['--headless=new', '--no-sandbox', '--disable-dev-shm-usage', '--disable-gpu', `--lang=${lang}`, `--accept-lang=${lang}`, `--remote-debugging-port=${port}`, `--user-data-dir=/tmp/nol-smoke-${port}`, '--window-size=1280,900', 'about:blank'], { stdio: ['ignore', 'ignore', 'pipe'] }); // --no-sandbox: Chrome's sandbox can't start inside the conveyor's macOS sandbox
  ...
  if (!ver) { console.log('could not start Chrome; stderr tail:\n' + stderrTail); process.exit(2); }
  ...
  for (const p of pages) {
    ...
    try {
      text = await Promise.race([
        ...
        sleep(60000).then(() => Promise.reject(new Error(`timeout: page ${p} [${lang}] did not finish in 60s (Chrome or CDP hung)`)))
      ]);
    } catch (e) { console.log('✖ ' + e.message); chrome.kill(); srv.close(); process.exit(2); }
